...
public class ChamadoDAO {
    ...
    public ChamadoModel createChamado(ChamadoModel chamado) throws SQLException {
        String sql = "INSERT INTO Task (titulo, descricao, dataCriacao, dataAtualizacao, dataConclusao, status, tipoDeServico, departamento, resposta, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        try (PreparedStatement ps = connection.prepareStatement(sql, Statement.RETURN_GENERATED_KEYS)) {
            ps.setString(1, chamado.getTitulo());
            ps.setString(2, chamado.getDescricao());
            ps.setDate(3, chamado.getDataCriacao());
            ps.setDate(4, chamado.getDataAtualizacao());
            ps.setDate(5, chamado.getDataConclusao());
            ps.setString(6, chamado.getStatus());
            ps.setString(7, chamado.getTipoDeServico());
            ps.setString(8, chamado.getDepartamento());
            ps.setString(9, chamado.getResposta());
    ...
    public boolean updateChamado(ChamadoModel chamado) throws SQLException {
        String sql = "UPDATE Task SET titulo=?, descricao=?, dataCriacao=?, dataAtualizacao=?, dataConclusao=?, status=?, tipoDeServico=?, departamento=?, resposta=?, user_id=? WHERE id=?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, chamado.getTitulo());
            ps.setString(2, chamado.getDescricao());
            ps.setDate(3, chamado.getDataCriacao());
